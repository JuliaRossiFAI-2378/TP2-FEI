...
function newsment_enqueue_child_styles()
{
    ...
    $newsment_version = wp_get_theme()->get('Version');
    ...
    wp_enqueue_style($parent_style, get_template_directory_uri() . '/style' . $min . '.css', array(), $newsment_version);
    wp_enqueue_style(
        'newsment',
...
function agregar_video_fondo() {
    ?>
    <video autoplay muted loop playsinline id="bg-video">
        <source src="http://localhost/wordpress/wp-content/uploads/2025/10/videoplayback.mp4" type="video/mp4">
    </video>
    <style>
    #bg-video {
        position: fixed;
...
add_action('wp_footer', 'agregar_video_fondo');
